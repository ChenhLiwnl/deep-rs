...
class PairedUpsDataset(Dataset):

    def __init__(self, datasets, patch_ratio, on_the_fly=True, patch_size=1000, num_patches=200, transform=None):
        super().__init__()
        self.datasets = datasets
        self.len_datasets = sum([len(dset) for dset in datasets])
        self.patch_ratio = patch_ratio
    ...
    def make_patches_for_ups_pair(self, pcl_A, pcl_B, pcl_C,patch_size, num_patches, ratio):
        """
        Args:
           pcl_A:  The first point cloud, (N, 3).
        pcl_B:  The second point cloud, (rN, 3).
        ...
        _, _, pat_B = pytorch3d.ops.knn_points(seed_pnts, pcl_B.unsqueeze(0), K=int(ratio*patch_size), return_nn=True)
        pat_B = pat_B[0]
        _, _, pat_C = pytorch3d.ops.knn_points(seed_pnts, pcl_C.unsqueeze(0), K=int(ratio*patch_size), return_nn=True)
        pat_C = pat_C[0]
    ...
    def __getitem__(self, idx):
        ...
        pat_low, pat_noisy , pat_gt= self.make_patches_for_ups_pair(
                pcl_data['original'],
                pcl_data['ups'],
                pcl_data['gt'],
                patch_size=self.patch_size,
        ...
        '''
        #pat_low = pcl_data['original']  
        pat_low = pat_low[0]      
        pat_noisy = pat_noisy[0]
        pat_gt = pat_gt[0]
        '''
        pat_low = pat_low[0]
        #patch?
        pat_noisy = pcl_data['down']
        pat_gt = pcl_data['gt']
        '''
        data = {
                'pcl_low': pat_low,
