...
class RandomRotate(object):
    ...
    def __call__(self, data):
        ...
        '''
        for k  in data:
            if k != 'name' and k != 'center' and k != 'scale':
                data[k] = torch.matmul(data[k],matrix)
...
class add_varying_noise(object):
    ...
    def __call__(self, data):
        noise_std = random.uniform(self.noise_std_min, self.noise_std_max)
        N = data['pcl_clean'].shape[0]
        p_max = data['pcl_clean'].max(dim=0, keepdim=False)[0]
        p_min = data['pcl_clean'].min(dim=0, keepdim=False)[0]
        center = (p_max + p_min) / 2    # (3,)
        noise_top = torch.FloatTensor(np.random.laplace(0, noise_std, size=data['pcl_clean'].shape)).to(data['pcl_clean'])
        phi = np.random.uniform(0, 2*np.pi, size=N)
        costheta = np.random.uniform(-1, 1, size=N)
        u = np.random.uniform(0, 1, size=N)
        theta = np.arccos(costheta)
        ...
        noise_bottom= np.zeros([N, 3])
        noise_bottom[:, 0] = r * np.sin(theta) * np.cos(phi)
        noise_bottom[:, 1] = r * np.sin(theta) * np.sin(phi)
        noise_bottom[:, 2] = r * np.cos(theta)
        noise_bottom = torch.FloatTensor(noise_bottom).to(data['pcl_clean'])

        idx_top = data['pcl_clean'][:,0]>center[0]
        idx_bottom = data['pcl_clean'][:,0] <= center[0]
        noise_top = torch.mul(noise_top , idx_top.unsqueeze(-1).repeat(1,3))
        noise_bottom = torch.mul(noise_bottom , idx_bottom.unsqueeze(-1).repeat(1,3))
        data['pcl_noisy'] = data['pcl_clean'] + noise_top + noise_bottom
        data['noise_std'] = noise_std
        return data
